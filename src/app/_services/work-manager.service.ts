...
export class WorkManagerService {
  ...
  private workStates: {[key: number]: CommandStacks};
  constructor() {
    this.WORK_WRAPPERS = {};
    this.active = null;
  ...
  setWorkStates(workStates: {[key: number]: CommandStacks}) {
  ...
  addWorkWrapper(workWrapper: WorkWrapperComponent) {
    ...
    workWrapper.work = this.assignWork(i, document.getElementsByClassName('canvasWrapper')[i]);
    workWrapper.work.setCommandStacks(this.workStates[i]);
    workWrapper.work.init();
  }
  ...
  assignWork(id: number, parentElement: Element): Work {
    ...
    return new WORKS[id % WORKS.length](parentElement);
  }
